"""
ClothesAdvisor.py creates a class ClothesAdvisor to test new images.
"""
...
class ClothesAdvisor:
	def __init__(self,  verbose=0):
		self.weights = "weights/finalWeights"
		self.ANN = self.establishANN()
		self.data = json.load(open('cluster/myData.json'))
	...
	def establishANN(self):
		"""
		Establish an artificial neural network and return it
		"""
		n = ANN()
	...
	def evaluateClothes(self, imageAddress, retsultFile = "cluster/result.json"):
		"""
		Evaluate a new image stored in imageAddress and save the recommendation in 
		cluster/result.json
		"""
		w = self.w
		h = self.h
		...
		if category:
			print "The category of this image is", category
		else:
			return
		try:
	...
	def getRecommended(self, category, in_clusters):
		"""
		Return items in the same cluster
		"""
	...
	def compareWithClusters(self, colorScheme, category):
		"""
		Compare a color scheme with all the cluster color schemes in a category
		"""
		hist = self.data[category]["hist"]
		vecList = []
	...
	def getColorDistribution(self, image, category):
		"""
		For a new image, get the color distrinbution 
		"""
	...
	def minDistInList(self, vec, listVec):
		"""
		Compare a vector with a list of vector, return the min distance,
		the label of the min distance, and a list stores all the distance
		"""
		label = -1
	...
	def getClothesCategory(self, imageAddress, inputFile = "inputs/all.dat"):
		"""
		Use ANN to recognize the category of new image input
		"""
	...
	def getInput(self, imageAddress, inputFile):
		"""
		Convert a image into the format we can pass into ANN
		"""
...
result = advisor.evaluateClothes("img/test-1.jpg")
