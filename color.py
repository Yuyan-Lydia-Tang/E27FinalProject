"""
Use kmeans clustering to classify the color schemes of images in each category and 
produce a json file called myData.json that stores the clustering information
"""
...
class colorSchemeCategorizer:
	...
	def getClusters(self, targetFile = "inputs/all.dat", imageFilenameRoot = "img/lg-"):
		...
		for article_index in range(diversity):
			...
			histRet,histLabel,histCenterList=cv2.kmeans(allHist,self.P,criteria,10,cv2.KMEANS_RANDOM_CENTERS)
	...
	def writeJson(self, dFile = "cluster/myData.json"):
		self.getMaxDistance()
		with open(dFile, 'w') as datafile:
			json.dump(self.data, datafile)

	def writeVisJson(self, dFile = "cluster/visualization.json"):
